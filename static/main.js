		...
		btn.onclick = async () => {
			...
			if (btn._audioInstance) {
				...
				if (!btn._playing) {
					btn._playing = true;
					btn.textContent = '⏹️';
					audio.onended = () => { btn._playing = false; btn.textContent = '🔊'; };
					audio.onerror = (e) => { btn._playing = false; btn.textContent = '🔊'; console.error('Audio playback error:', e); };
					audio.play().catch(e => { btn._playing = false; btn.textContent = '🔊'; console.error('Audio play failed:', e); });
				} else {
...
async function sendText() {
	...
	if (stagedFile) {
		...
		const res = await fetch(`${apiBase}/upload-file`, { method: 'POST', headers, body: form });
		const data = await res.json();
		sessionId = data.session_id;
		addMessage('ai', data.reply_text, data.reply_audio_url);
		stagedFile = null;
		if (stagedFileEl) { stagedFileEl.style.display = 'none'; stagedFileEl.textContent = ''; }
		loadHistory();
		return;
	}
	...
	const res = await fetch(`${apiBase}/send-message`, {
		method: 'POST',
		headers,
		body: JSON.stringify({ session_id: sessionId, input_type: 'text', content: text })
	});
	const data = await res.json();
	sessionId = data.session_id;
	addMessage('ai', data.reply_text, data.reply_audio_url);
...
async function loadHistory() {
	if (!sessionId) return;
	const res = await fetch(`${apiBase}/history?session_id=${encodeURIComponent(sessionId)}`);
	const data = await res.json();
...
async function uploadSelectedFile(file) {
	...
	if (file && file.type && file.type.startsWith('audio/')) {
		...
		const tres = await fetch(`${apiBase}/transcribe`, { method: 'POST', body: tform });
	...
	const res = await fetch(`${apiBase}/upload-file`, { method: 'POST', headers: voiceHeaders, body: form });
	const data = await res.json();
	sessionId = data.session_id;
	addMessage('ai', data.reply_text, data.reply_audio_url);
	loadHistory();
...
if (settingsSave) settingsSave.onclick = () => { saveSettings(); closeSettings(); };
